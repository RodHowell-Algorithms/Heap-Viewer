...
public final class BinomialHeap implements Serializable {
	 ...
	 */
	private GenericConsList<BinomialHeap> children;
	...
	public BinomialHeap(int p) {
		...
		children = new GenericConsList<BinomialHeap>();
		rank = 0;
	...
	public BinomialHeap(BinomialHeap h1, BinomialHeap h2) throws IllegalArgumentException {
		...
		root = larger.root;
		children =
		  new GenericConsList<BinomialHeap>(smaller, larger.children);
		rank = h1.rank + 1;
	 ...
	 * @return The children.
	 */
	public GenericConsList<BinomialHeap> getChildren() {
		return children;
	}
	...
	public TreeDrawing getDrawing() {
		if (drawing == null) {
			...
			GenericConsList<BinomialHeap> lst = children;
			for (int i = rank-1; i >= 0; i--) {
				d[i] = lst.getHead().getDrawing();
				lst = lst.getTail();
			}
